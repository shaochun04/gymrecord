...
const dumbbell: ExerciseDefinition = { id: 'dumbbell-bench', name: '臥推', equipment: '啞鈴', variation: '平板', primaryMuscles: ['chest'], secondaryMuscles: ['triceps'], archived: false }
const barbell: ExerciseDefinition = { ...dumbbell, id: 'barbell-bench', equipment: '槓鈴' }
const smith: ExerciseDefinition = { ...dumbbell, id: 'smith-bench', equipment: '史密斯' }
const incline: ExerciseDefinition = { ...dumbbell, id: 'incline-bench', variation: '上斜' }
...
const exercise = (id: string, definitionId: string, sets: SetLog[], name = '臥推'): SessionExercise =>
  ({ id, sourceExerciseId: `slot-${id}`, exerciseDefinitionId: definitionId, name, equipment: '啞鈴', variation: '平板', note: '椅背 30°', restSeconds: 90, targetRepsMin: 8, targetRepsMax: 12, sets })
...
describe('global exercise identity and history', () => {
  const older = session('push', '2026-09-20T10:00:00Z', [exercise('push-slot', dumbbell.id, [set('old', 20, 10)])])
  const newer = session('upper', '2026-09-25T10:00:00Z', [exercise('upper-slot', dumbbell.id, [set('a', 22, 10), set('b', 22, 9, true, 'working', 2), set('warm', 10, 12, true, 'warmup'), set('undone', 30, 5, false)])])

  it('shares latest completed working sets across Push and Upper by definition ID', () => {
    const active = session('active', '2026-09-26T10:00:00Z', [exercise('active', dumbbell.id, [set('active-set', 30, 8)])], 'active')
    expect(findPreviousPerformance(dumbbell.id, [older, active, newer])?.sets.map((row) => row.id)).toEqual(['a', 'b'])
    const history = findExerciseHistory(dumbbell.id, [older, active, newer])
  ...
  it('does not combine barbell, dumbbell, and Smith even with the same display name', () => {
    const mixed = session('mixed', '2026-09-26T10:00:00Z', [
      exercise('barbell', barbell.id, [set('barbell', 80, 5)]),
      exercise('smith', smith.id, [set('smith', 90, 5)]),
      exercise('dumbbell', dumbbell.id, [set('dumbbell', 22, 10)]),
    ])
    expect(calculateExercisePr(findExerciseHistory(dumbbell.id, [mixed])).weightPr?.id).toBe('dumbbell')
    expect(calculateExercisePr(findExerciseHistory(barbell.id, [mixed])).weightPr?.id).toBe('barbell')
    expect(calculateExercisePr(findExerciseHistory(smith.id, [mixed])).weightPr?.id).toBe('smith')
  })

  it('keeps different variations separate even when name and equipment match', () => {
    const mixed = session('mixed', '2026-09-26T10:00:00Z', [
      exercise('flat', dumbbell.id, [set('flat-set', 22, 10)]),
      exercise('incline', incline.id, [set('incline-set', 18, 10)]),
    ])
    expect(findPreviousPerformance(dumbbell.id, [mixed])?.sets[0].id).toBe('flat-set')
    expect(findPreviousPerformance(incline.id, [mixed])?.sets[0].id).toBe('incline-set')
  })

  it('keeps archived definitions queryable and history copies isolated', () => {
    const archived = { ...dumbbell, archived: true }
    const previous = findPreviousPerformance(archived.id, [older])!
    previous.sets[0].weight = 99
    expect(older.exercises[0].sets[0].weight).toBe(20)
    expect(findPreviousPerformance(archived.id, [older])?.sets[0].weight).toBe(20)
  })
  ...
  it('prefills each set from latest global history, snapshots definition and targets, and never copies RIR', () => {
    const routine: Routine = { id: 'push', name: 'Push', label: '', accent: '#fff', order: 0, updatedAt: '2026-09-01T00:00:00Z',
      exercises: [{ id: 'push-slot', exerciseDefinitionId: dumbbell.id, weight: 15, reps: 8, targetRepsMin: 8, targetRepsMax: 12, sets: 3, restSeconds: 90, note: '椅背 30°' }] }
    const currentDefinition = { ...dumbbell }
    const started = makeSession(routine, [currentDefinition], [older, newer])
    expect(started.exercises[0]).toMatchObject({ exerciseDefinitionId: dumbbell.id, sourceExerciseId: 'push-slot', name: '臥推', equipment: '啞鈴', variation: '平板', targetRepsMin: 8, targetRepsMax: 12 })
  ...
  it('preserves PR calculation regardless of RIR', () => {
    const history = findExerciseHistory(dumbbell.id, [older, newer])
...
describe('targets, progression, rest and CSV', () => {
  ...
  it('keeps kg storage and RIR/targets in CSV', () => {
    ...
    const finished = session('finished', '2026-09-25T10:00:00Z', [exercise('e', dumbbell.id, [set('a', 20, 10, true, 'working', 4), set('b', 10, 12, true, 'warmup')])])
    expect(completedWorkingVolumeKg(finished)).toBe(200)
    expect(serializeCsv([finished])).toContain('"20","10","是","4","8","12"')
