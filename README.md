...
- 從全域動作庫挑選動作，管理器材、變化、主要／次要肌群及封存狀態。
...
- 訓練中自動儲存；關閉或重新整理後可繼續未完成的 Session。
- 查看訓練日曆、逐組歷史、重量 PR、同重量次數 PR 與 Epley 估算 1RM。
- 查看每週主要肌群有效組數、訓練天數及未分類組數。
- 修正已完成訓練的重量、次數、RIR、完成狀態及組別類型，統計會自動重算。
...
請定期從設定頁匯出 JSON 備份。換手機、清除 Chrome 網站資料或移除 PWA 前，應先匯出並確認備份檔可讀。手機與電腦之間可透過 JSON 匯出／匯入轉移完整資料。目前備份格式為 v3，仍可匯入 v1 與 v2 備份。
